...
if page == "Đăng Ký Sinh Viên":
    ...
    if selected_session:
        ...
        if attendance_method == "Chụp ảnh":
            ...
            st.write("Chức năng này sẽ tự động quét khuôn mặt từ camera để điểm danh.")
            ...
            # Tạo placeholder để hiển thị khung hình camera
            camera_placeholder = st.empty()
            
            # Tạo nút dừng để thoát vòng lặp
            ...
            # Vòng lặp hiển thị khung hình camera
            while st.session_state['scanning']:
                ...
                if image is not None:
                    ...
                    if img_array is not None:
                        ...
                        # Hiển thị khung hình camera trong placeholder
                        camera_placeholder.image(img_array, caption="Đang quét khuôn mặt...", use_column_width=True)
                        
                        # Xử lý nhận diện khuôn mặt và điểm danh
                        faces = recognizer.app.get(img_array)
                        if len(faces) == 1:
                            embedding = faces[0].embedding
                            record_id, student_id, student_name = find_closest_match(embedding, record_ids, ids, names, embeddings)
                ...
                # Giảm tần suất cập nhật để tránh tải nặng
                time.sleep(0.1)
